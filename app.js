import { allDocs, dossierDocs, emails } from "./data.js";
import { clearSession, loadSession, saveSession, tryAdminUnlock, tryLogin } from "./auth.js";
import { escapeHtml, extractNotes, isProtected, matchesSearch, stripNotes } from "./utils.js";
      ...
      <div class="card grid-login">
        <div class="login-left">
          ...
          <div class="inline-box" style="margin-top:28px; background: rgba(0,0,0,0.2);">
            ...
            <div class="reader-pre" style="font-family: var(--font-mono); margin-top:10px; color: rgba(230,247,255,0.8);">
Acesso restrito. Toda autenticação é registrada. Tentativas inválidas podem acionar auditoria interna.
            </div>
          </div>
        </div>
...
function renderArchive() {
          ...
          <div class="archive-grid">
            <div class="left-column">
              <div class="search-box">
                <div class="h2" style="font-size:18px; display:flex; align-items:center; gap:10px;">${iconSvg("search")} Pesquisa e filtros</div>
                <div class="search-input-wrap" style="margin-top:16px;">
                  <span class="icon">${iconSvg("search")}</span>
                  <input id="search-input" class="input search-input" placeholder="Buscar por código, lote, autor ou conteúdo..." value="${escapeHtml(state.query)}" />
                </div>
              ...
              <div>
                <div class="section-title">Arquivos localizados</div>
                <div class="docs-list" style="margin-top:12px;">
                  ${filtered.length === 0 ? `<div class="no-files">Nenhum arquivo corresponde à busca atual.</div>` : filtered.map(renderDocCard).join("")}
                </div>
              </div>
            </div>
  ...
  document.querySelectorAll("[data-doc-id]").forEach((el) => {
    el.addEventListener("click", () => {
      state.selectedId = el.getAttribute("data-doc-id");
      renderArchive();
    });
  });
...
function statBox(icon, label, value) {
  return `
    <div class="stat-box">
      <div class="stat-row">${iconSvg(icon)} <span>${label}</span></div>
  ...
  if (state.session.level === "admin") {
    return `
      <div class="reader-side-box green">
        ...
        <div class="muted" style="margin-top:10px; line-height:1.8; color:#dbfff0;">
          Todos os arquivos sensíveis protegidos estão liberados nesta sessão.
        </div>
      </div>
    `;
  }
  ...
  return `
    <div class="reader-side-box amber">
      ...
      <div class="muted" style="margin-top:10px; line-height:1.8;">
        Alguns e-mails e documentos sensíveis exigem autenticação elevada para leitura completa.
      </div>
